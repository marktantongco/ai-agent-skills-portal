...
export const INSTALLED_SKILLS: Skill[] = [
  ...
  { name: "motion-system-playbook", category: "Design/UI", description: "Animation governance across 11 libraries — selection matrix, universal rules, accessibility, combo safety, testing QA", source: "built-in", installs: "built-in", isNew: true, healthScore: 95 },
]
...
export const SKILL_COMBOS: SkillCombo[] = [
  ...
  { name: "Motion System", emoji: "🎬", skills: ["motion-system-playbook", "gsap-animations", "frontend-design", "react-best-practices"], tagline: "Govern → Animate → Design → Perform", useCase: "Production-grade animations that are accessible, performant, and safe across libraries", synergy: "motion-system-playbook governs the rules and library selection, gsap-animations provides the implementation, frontend-design ensures visual quality, react-best-practices prevents performance regressions.", whyChosen: "Animations fail at governance: wrong library, no reduced-motion, layout thrashing. Each skill covers one layer.", benefitsVs: "WITHOUT: Random animation choices, accessibility violations, 50% frame drops. WITH: Governed selection, 60fps guaranteed, reduced-motion compliant, library-safe combos.", misconceptions: "'Just use GSAP for everything.' Reality: GSAP is 50KB for a hover effect. CSS is 0KB. Solution: Use the library selection matrix." },
]
...
export const PLAYBOOKS: Playbook[] = [
  ...
  { name: "Motion Audit", emoji: "🎭", description: "Audit and fix any animation system for performance, accessibility, and library safety", chain: ["motion-system-playbook", "gsap-animations", "react-best-practices", "output-formatter"], trigger: "/motionaudit", copyText: "Run: motion-system-playbook → gsap-animations → react-best-practices → output-formatter", whyItWorks: "Playbook governs the audit rules, GSAP provides implementation patterns, React best practices catches performance issues, formatter produces the audit report." },
]
...
export const COMPATIBILITY: Compatibility[] = [
  ...
  { type: 'synergy', skillA: 'motion-system-playbook', skillB: 'gsap-animations', reason: 'Playbook governs rules and library selection, GSAP provides the implementation engine' },
]
...
export const DEPENDENCIES: Dependency[] = [
  ...
  { skill: "motion-system-playbook", depends: ["gsap-animations", "frontend-design"], reason: "Playbook delegates implementation syntax to domain-specific animation skills" },
]
...
export const FAQ_DATA: FAQItem[] = [
  ...
  { question: "Can I create my own skill combinations?", answer: "Yes — the Combo Generator lets you pick any skills from the 83-skill directory, name your combo, and get a copy-ready command. The generator also checks for compatibility conflicts and suggests synergies based on existing stack patterns. You can export your custom combo in multiple formats: chain, list, command, or JSON." },
